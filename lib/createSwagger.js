...
function turnParamType2In(http, acceptItem) {
  ...
  if (acceptItem.http && acceptItem.http.source) {
    paramType = acceptItem.http.source === 'formData' ? 'formData' : acceptItem.http.source.toLowerCase();
  }
...
function returnsBuildFromAccept(returnsConfig) {
  ...
  const modelNameLowerCase = (modelName && typeof modelName === 'string') && modelName.toLowerCase();
...
function buildPathWithVerb(remoteConfig, methodName, ModelName) {
  ...
  if (!config.parameters && config.accepts) {
    ...
    const parameters = acceptsArr.map(item => {
      ...
      const paramIn = turnParamType2In(remoteConfig.http, item);
      const parameter = {
        in: paramIn,
...
function buildDefinitionWithModel(config, Model) {
  ...
  const definition = extendFilterDefaultOption(swaggerTemplate.defaultDefinitionItem, extConfig);
  // if required.length === 0 ignore required
  // swagger 校验 报错
