...
function buildTypeAndFormat(acceptItem) {
  ...
  switch (acceptItem.type) {
    case 'date':
      extConfig.type = 'string';
      extConfig.format = 'DATE';
      break;
    case 'buffer':
      extConfig.type = 'string';
      extConfig.format = 'BYTE';
      break;
    case 'number':
      extConfig.type = 'number';
      extConfig.format = acceptItem.format || 'int64';
      break;
    case 'integer':
      extConfig.type = 'integer';
      extConfig.format = acceptItem.format || 'int64';
      break;
    case 'object':
      extConfig.type = 'string';
      extConfig.format = 'JSON';
      break;
    default:
      extConfig.type = 'string';
  }
...
function returnsBuildFromAccept(remoteConfig) {
  ...
  const returnsConfig = remoteConfig.returns || {};
  ...
  if (modelName) {
    ...
    if (typeName === 'object' && returnsConfig.root) {
      ...
      const swaggerTypeAndFormat = defaultSwaggerTypeAndFormatFromDescType(typeName);
      res.schema = {
        type: 'object',
        properties: {
          [returnsConfig.arg]: {
            type: swaggerTypeAndFormat.type,
            format: returnsConfig.format || swaggerTypeAndFormat.format,
          },
        },
      };
    } else {
 ...
 * @return {object} json
 */
function init(Models, baseConfig) {
  //
  return swaggerRoot;
}
