...
class BaseController extends Controller {
   ...
   * @param {Boolean} isStatic 是否静态函数
   * @return {array} argument
   */
  static async _validateAndParseRemoteInput(ctx, accepts, isStatic) {
    const validateConfigs = helper.parseAcceptsToArgs(accepts);
    await helper.validateCtxInput(ctx, validateConfigs);

    return helper.parseCtxInput(ctx, accepts, isStatic);
  }
